...
class DoubleQSequential(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, name, chkpt_dir):
        super(DoubleQSequential, self).__init__()
        ...
        self.model1 = nn.Sequential(
            nn.Linear(num_inputs + num_actions, 50),
            nn.ReLU(),
            nn.Linear(50, 100),
            nn.ReLU(),
            nn.Linear(100, 200),
...
class DoubleQLeaky(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, name, chkpt_dir):
        super(DoubleQLeaky, self).__init__()
        if not os.path.exists(chkpt_dir): os.makedirs(chkpt_dir)
        self.chkpt_dir = chkpt_dir
        self.name = name
...
class GaussianPolicy(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, name, chkpt_dir, action_space=None):
        super(GaussianPolicy, self).__init__()
        if not os.path.exists(chkpt_dir): os.makedirs(chkpt_dir)
        self.chkpt_dir = os.path.join(chkpt_dir)
        self.name = name
        ...
        if action_space is None:
            ...
            self.action_scale = torch.FloatTensor(
                np.array(action_space.max(axis=1) - action_space.min(axis=1)) / 2.)
